...
ReactDOM.createRoot()接受三个参数，其中第一个参数比较重要

接收到字符串时，定义一个标签，

接收到一个函数时（不能是函数的结果），通常这个函数返回一个组件，他会调用这个函数，并实例化这个组件
...
ui = <Property['Access'] /> // SyntaxError 语法错误
...
<></>为 React.Fragment 的简写，可以避免生成额外的节点
## Background
